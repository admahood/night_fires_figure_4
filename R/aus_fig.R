...
inset_s <- ggplot(snowy_clim, aes(x=datetime, y=value)) +
  geom_line() +
  geom_abline(aes(slope = 0,intercept = 0.3, lty=as.factor(lty)))+
...
inset_t <- read_csv("data/tubbs-hourly.csv") %>% 
  ...
  xlab("Date") +
  scale_x_datetime(date_breaks = "1 week", date_labels = "%b %d")+
  geom_hline(aes(yintercept = 0.5, lty=as.factor(lty)))+
...
leg <- get_legend(snowy%>%
                    mutate(daynight = ifelse(daynight=="day", "Day", "Night"),
                           daynight = factor(daynight, levels = c("Night", "Day"))) %>%
                    ggplot() +
...
insets <- ggarrange(inset_s, inset_t, inset_b, 
                    ...
                    label.x = c(.8, .85, .35), label.y = .95, hjust="right", vjust="top")
...
finalfig <- ggdraw(xlim = c(0,7.5), ylim = c(0, 11.75)) +
  ...
  ggsave("images/panel_fig.png", height = 11.75, width = 7.5)
